...
bool CStateBloodsuckerVampireAbstract::check_start_conditions()
{
	...
	const CEntityAlive *enemy_ = this->object->EnemyMan.get_enemy();
	if (!smart_cast<CActor const*>(enemy_))			return false;
...
TEMPLATE_SPECIALIZATION
void CStateBloodsuckerVampireAbstract::remove_links	(CObject* object_)
{
	if (enemy == object_)
		enemy					= 0;
}
